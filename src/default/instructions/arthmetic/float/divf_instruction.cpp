...
DivFInstruction::DivFInstruction(Opcode opcode, std::any operand)
    : Operation(opcode, operand)
{}

void DivFInstruction::execute(IContext *context) {
    ...
    if(TypeTools::getType(std::type_index(operand1.type())) != Type::DOUBLE 
        ...
        std::cerr << "Invalid operands for instruction DivF" << std::endl;
    }
