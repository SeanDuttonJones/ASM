...
MulIInstruction::MulIInstruction(Opcode opcode, std::any operand)
    : Operation(opcode, operand)
{}

void MulIInstruction::execute(IContext *context) {
    ...
    if(TypeTools::getType(std::type_index(operand1.type())) != Type::INT 
        ...
        std::cerr << "Invalid operands for instruction MulI" << std::endl;
    }
